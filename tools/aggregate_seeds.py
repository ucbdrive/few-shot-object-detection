...
def parse_args():
    ...
    parser.add_argument(
        "--seeds", type=int, default=30, help="Seeds to aggregate over"
    )
    # Model
    parser.add_argument(
...
def main(args):  # noqa: C901
    ...
    for i in range(args.seeds):
        ...
        if os.path.exists(ckpt):
            if os.path.exists(os.path.join(ckpt, "inference/all_res.json")):
                ...
                os.path.join(ckpt, "inference/res_final.json")
            ):
    ...
    if args.print:
        ...
        for metric in metrics:
            out_str += "{0:.1f} ".format(
                1.96
                * np.std(metrics[metric])
                / math.sqrt(len(metrics[metric]))
            )
        print(out_str)
        out_str = ""
        for metric in metrics:
        ...
        for metric in metrics:
            ...
            print(
                "Median \t {0:.4f}".format(np.percentile(metrics[metric], 50))
            )
    ...
    if args.plot:
        ...
        for met in ["avg", "stdev", "ci"]:
            for metric, c in zip(
                ["nAP", "nAP50", "nAP75"], ["bo-", "ro-", "go-"]
            ):
                if met == "avg":
                    res = [
                        np.mean(metrics[metric][: i + 1])
                        for i in range(len(metrics[metric]))
                    ]
                elif met == "stdev":
                    res = [
                        np.std(metrics[metric][:i])
                        for i in range(1, len(metrics[metric]) + 1)
                    ]
                elif met == "ci":
                    res = [
                        1.96
                        * np.std(metrics[metric][: i + 1])
